...
angular.module('provinceApp').factory('GoogleRealtime', function($window, State, $rootScope, appConfig) {
  ...
  var realtime = {
    ...
    addScript: function() {
      ...
      po.src = 'https://apis.google.com/js/api.js?onload=handleApiLoad';
    ...
    handleDriveApiLoad: function() {
      ...
      gapi.drive.realtime.load(realtime.fileId, realtime.handleModelLoad);
    },

    // getRoot() returns https://developers.google.com/drive/realtime/reference/gapi.drive.realtime.CollaborativeMap
    // get('listName') returns https://developers.google.com/drive/realtime/reference/gapi.drive.realtime.CollaborativeList
    handleModelLoad: function(model) {

      var list = model.getModel().getRoot().get(realtime.collaborativeListName);

      list.addEventListener(gapi.drive.realtime.EventType.OBJECT_CHANGED, function() {
        $rootScope.$emit('clientsChanged', list.asArray());
      });

      var userAgent = navigator.userAgent; // User agent for now but ip later

      if (list.indexOf(userAgent) === -1) {
        list.push(userAgent);
      }
    },
    ...
    createNewRealtimeFile: function() {
      realtime.createRealtimeFile(realtime.fileName, realtime.fileMimeType, function(file) {
        ...
        gapi.drive.realtime.load(file.id, function() {}, function(model) {
